...
const defaultAlbums = [
  { id: "1", name: "Architecture & Urban", count: 4, cover: "https://images.unsplash.com/photo-1513694203232-719a280e022f?auto=format&fit=crop&w=600&q=80", tag: "architecture" },
  { id: "2", name: "Nature & Landscapes", count: 3, cover: "https://images.unsplash.com/photo-1506744038136-46273834b3fb?auto=format&fit=crop&w=600&q=80", tag: "nature" },
  { id: "3", name: "Cyberpunk & Neon", count: 2, cover: "https://images.unsplash.com/photo-1519501025264-65ba15a82390?auto=format&fit=crop&w=600&q=80", tag: "cyberpunk" },
  { id: "4", name: "Minimalist Interiors", count: 3, cover: "https://images.unsplash.com/photo-1494438639946-1ebd1d20bf85?auto=format&fit=crop&w=600&q=80", tag: "minimalist" },
];
...
export default function AlbumsPage() {
  ...
  // Load persistent albums from localStorage
  useEffect(() => {
  ...
  // Save albums to localStorage whenever changed
  const saveAlbums = (updatedAlbums: typeof albums) => {
    setAlbums(updatedAlbums);
    localStorage.setItem("lumina_albums", JSON.stringify(updatedAlbums));
  };
  ...
  const handleCreateAlbum = (e: React.FormEvent) => {
    ...
    const newFolder = {
      ...
      name: newAlbumName,
      count: 0,
      cover: "", // Empty cover for new folders
      tag: tag,
    };

    saveAlbums([...albums, newFolder]);
    setNewAlbumName("");
  ...
  return (
    <div className={`min-h-screen flex flex-col ${isDark ? "bg-[#09090b] text-zinc-100" : "bg-white text-zinc-900"}`}>
      ...
      <Dialog open={createModalOpen} onOpenChange={setCreateModalOpen}>
        <DialogContent className={`max-w-md rounded-3xl p-6 ${isDark ? "bg-[#09090b] border-zinc-800 text-white" : "bg-white border-zinc-300 text-zinc-900"}`}>
          <DialogHeader className="space-y-1">
            <DialogTitle className="text-lg font-bold">Create New Album Folder</DialogTitle>
          ...
          <form onSubmit={handleCreateAlbum} className="space-y-4 pt-3">
            <div className="space-y-1">
              <label className="block text-[10px] font-mono uppercase text-zinc-400">Album Name</label>
              <Input
                type="text"
                required
                ...
                className={isDark ? "bg-zinc-900 border-zinc-800 text-white" : "bg-zinc-100 border-zinc-300 text-zinc-900"}
              />
            </div>
