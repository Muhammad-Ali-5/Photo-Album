...
export default function Slider({children}:any) {
  ...
  useEffect(() => {
    ...
    console.log(`Theme : ${stored_theme}`)
    ...
    useEffect(() => {
      if(localStorage.getItem("theme")!==null){
        console.log(`Dark theme not null changing : ${dark_theme}`)
    ...
    <div className={`${dark_theme?"dark bg-gray-950":"text-black"} overflow-scroll overflow-x-hidden h-full  text-white  flex dark flex-col`}>
      <div className='fixed z-50 bg-gray-900'>
        <Navbar dark_theme={dark_theme} setdark={setdark} dialog={dialog} setdialog={setdialog} />
        {/* <Navbar /> */}
      </div>

        <div className='mt-14 flex w-full h-full'>

          <div className={`${hov?("z-40"):(mob?(dialog?("z-40"):("z-10 delay-100")):("z-10 delay-100"))} h-full fixed`}>

        <Side_Nav dark_theme={dark_theme} setdark={setdark} dialog={dialog} setdialog={setdialog} mob={mob} hov={hov} setmob={setmob} sethov={sethov} />
        {/* <Side_Nav /> */}
          </div>
